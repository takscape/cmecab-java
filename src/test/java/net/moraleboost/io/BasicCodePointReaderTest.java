...
public class BasicCodePointReaderTest
{
    ...
    {
        int scp = 0x00010400;
        int[] answer = new int[] { scp, 'あ', 'a', 'い', scp, scp, 'd', 'お', scp };
        long[] positions = new long[] { 0, 2, 3, 4, 5, 7, 9, 10, 11, 13 };
    ...
    public void testEndWithHighSurrogate() throws IOException
    {
        ...
        int[] answer = getCodePoints(base
                + (char)BasicCodePointReader.DEFAULT_ALTERNATION_CODEPOINT);
        long[] positions = getPositions(base
                + (char)BasicCodePointReader.DEFAULT_ALTERNATION_CODEPOINT);
        CodePointReader reader = new BasicCodePointReader(new CharArrayReader(
                writer.toCharArray()));
        if (!match(reader, answer, positions)) {
            fail("コードポイントが一致しません。");
        }
    ...
    public void testStartWithHighSurrogate() throws IOException
    {
        ...
        int[] answer = getCodePoints((char)BasicCodePointReader.DEFAULT_ALTERNATION_CODEPOINT
                + base);
        long[] positions = getPositions((char)BasicCodePointReader.DEFAULT_ALTERNATION_CODEPOINT
                + base);
        CodePointReader reader = new BasicCodePointReader(new CharArrayReader(
                writer.toCharArray()));
        if (!match(reader, answer, positions)) {
    ...
    {
        int scp = 0x00010400;
        int[] original = new int[] { 'あ', 'a', 'い', scp, scp, 'd', 'お' };
        // high surrogateを破壊した場合の正解
        int[] answer1 = new int[] { 'あ', 'a', 'い', 'a',
                BasicCodePointReader.DEFAULT_ALTERNATION_CODEPOINT, scp, 'd',
                'お' };
        long[] positions1 = new long[] { 0, 1, 2, 3, 4, 5, 7, 8, 9 };
        // low surrogateを破壊した場合の正解
        int[] answer2 = new int[] { 'あ', 'a', 'い',
                BasicCodePointReader.DEFAULT_ALTERNATION_CODEPOINT, 'a', scp,
                'd', 'お' };
        long[] positions2 = new long[] { 0, 1, 2, 3, 4, 5, 7, 8, 9 };
        ...
        if (!match(new BasicCodePointReader(new StringReader(ill1)), answer1,
                positions1)) {
            fail("Low surrogateが単独で存在する場合のコードポイントが一致しません。");
        }
        if (!match(new BasicCodePointReader(new StringReader(ill2)), answer2,
                positions2)) {
